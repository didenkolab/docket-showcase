---
key: HARBOR-82
title: Reconcile the provider's webhook with the booking
type: story
status: QA
status_category: doing
priority: normal
assignee: '[[tomasz]]'
parent: "[[HARBOR-3 Card payments]]"
labels: ["[[payments]]"]
created: 2026-07-13T09:45:00Z
updated: 2026-07-22T10:20:00Z
aliases: []
tags: [area/api]
definition_of_done: team
sprint: "[[Sprint 3]]"
estimate: 5
mitigates: ["[[HARBOR-65 Postmortem - the double charge of 8 July]]"]
---

The provider tells us what happened twice: once as the answer to our request and once, minutes or hours later, as a webhook. When those two disagree the webhook is right, and today nothing is listening to it.

## Acceptance

- [x] A webhook the provider sends twice is applied once
- [x] A charge the webhook reports and the booking does not know about raises an alert
- [x] A webhook for a booking that no longer exists is kept, not dropped

## Comments
