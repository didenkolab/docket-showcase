---
key: HARBOR-103
title: Check a guest in from the pontoon with no signal
type: story
status: Done
status_category: done
priority: normal
assignee: '[[aiko]]'
parent: "[[HARBOR-4 Mobile check-in]]"
labels: ["[[mobile]]"]
created: 2026-07-27T09:40:00Z
updated: 2026-08-14T16:53:00Z
aliases: []
tags: []
definition_of_done: release
sprint: "[[Sprint 5]]"
tested_by: ["[[HARBOR-123 A guest is checked in with no signal to check them in on]]"]
logged: ["[[HARBOR-146 Queue flush against a phone with no signal]]", "[[HARBOR-147 Showing the queued state on the check-in screen]]", "[[HARBOR-149 Pontoon flow written up for review]]", "[[HARBOR-150 Wet-hands pass over the check-in screen]]", "[[HARBOR-151 Check-in tested on the pontoon at Vik]]", "[[HARBOR-152 Fixing what came back from the pontoon]]", "[[HARBOR-154 Second pass, and the papers photograph]]", "[[HARBOR-155 Closing the pontoon story]]", "[[HARBOR-177 Check the launch build on both marina phones]]"]
blocks: ["[[HARBOR-143 Scan the berth's QR code to open the right booking]]"]
---

The dockhand meeting the boat has a phone, a wet hand and one bar of signal that comes and goes behind the fuel dock. Check-in has to complete on the pontoon and reconcile later — an app that needs a connection at the moment the boat arrives is an app the marina will stop opening.

## Acceptance

- [x] A check-in completes with the phone in flight mode
- [x] The office sees it within a minute of the phone reconnecting
- [x] Two dockhands checking in the same boat produce one check-in, not two

## Comments
