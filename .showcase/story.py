...
SPRINTS: list[Sprint] = [
    Sprint(
        ...
        retro=(
            ...
            "the sprint was for. Reserving a berth for a date range (`{harbor.booking.week}`), "
            "the free-berths answer the harbour office actually asks for "
            "(`{harbor.booking.availability}`), and importing a real marina's forty-one berths "
            "(`{harbor.booking.import-berths}`) all finished inside the two weeks. Ledgerline's "
            "invoice numbering landed beside them, so the demo we gave on the Friday was one "
            "flow across two products rather than two demos.\n\n"
            "The one that hurt was the invoice story (`{harbor.booking.invoice}`) coming back "
            "from review on the Thursday: it priced the booking at today's rate rather than the "
            "rate on the day it was "
    ...
    Sprint(
        ...
        retro=(
            ...
            "the two. The twenty-minute hold (`{harbor.booking.hold}`) went in beside it, which "
            "is what stops two people buying the same berth while one of them is typing a card "
            "number.\n\n"
            "It was not a clean two weeks. The card-payment story (`{harbor.payments.capture}`) "
            "failed in QA on the Thursday: a declined card rolled back the charge and not the "
            "booking, so the berth stayed held for a guest who had not paid \u2014 the failure "
            "path had been tested by reasoning about it rather than by declining a real sandbox "
            "card. On top of that, the double-booking bug (`{harbor.booking.double-book}`) "
            "found on the first day of the sprint took a day and a half because the "
            "availability check and the write were two separate reads with nothing between "
            "them, and fixing it properly meant changing how a hold is created rather than "
            "adding a lock. Ola also found the sandbox keys sitting in the repository "
            "(`{harbor.payments.provider-keys}`), which is a sprint's worth of unpleasantness "
            "we bought ourselves in Sprint 1.\n\n"
    ...
    Sprint(
        ...
        retro=(
            ...
            "carried on underneath: the webhook reconciliation (`{harbor.payments.webhook}`), "
            "the month-query cache (`{harbor.calendar.cache}`) that took the calendar from two "
            "and a half seconds to under one, and the calendar bug (`{harbor.calendar.left-yesterday}`) "
            "that drew boats which had left the day before as still moored.\n\n"
            "The double charge (`{harbor.payments.double-charge}`) on the Wednesday of the "
            "first week was the sprint's real event. Three marinas, twenty minutes apart, all "
            "with the same shape: a slow confirmation, a guest pressing the button again, two "
            "charges and one booking. It is written up as an incident with the refunds against "
            "it. What made it possible was that our confirmation had no way of saying 'this is "
            "the same intention as the last one', and it was found by three customers rather "
            "than by us. The calendar month story (`{harbor.calendar.month}`) also came back "
            "from review \u2014 beautiful on forty berths and unusable on four hundred and six, "
            "which is what Sandholm have.\n\n"
    ...
    Sprint(
        ...
        retro=(
            ...
            "the same trick from it: the queued check-in (`{harbor.checkin.pontoon.1}`) and the "
    ...
    Sprint(
        ...
        retro=(
            ...
            "(`{harbor.checkin.qr}`) that opens the right berth's booking, and the photograph "
            "of the boat's papers (`{harbor.checkin.photo}`) all shipped, and the whole "
            "check-in epic is finished. Ola had both phones on the build a week before the "
            "launch instead of on the Friday, which is the only reason the weekend was "
            "boring.\n\n"
            "Two things we would rather had gone differently. The old test phone's clock is "
            "eleven minutes fast and queued check-ins carried it (`{harbor.checkin.clock-skew}`), "
            "so the office saw arrivals out of order and twice in the future \u2014 found on the "
            "Thursday, fixed by the Tuesday, but it is the kind of thing a launch weekend "
            "should not be discovering. And season pricing (`{harbor.booking.season-rate}`) "
            "has been sitting in review since the middle of the sprint waiting for us to decide "
            "what happens when a season berth is cut short, which is a product question that a "
            "card in review cannot answer on its own.\n\n"
            "The kiosk (`{harbor.checkin.kiosk}`) was cancelled during the sprint rather than "
            "carried: it is the same job as the sticker, and saying so cost one conversation "
            "with Sandholm. We should do that earlier and more often. Next sprint starts by "
            "taking a decision on the shortened-season refund rather than by pulling more work "
            "in beside it."
        ),
    ),
...
class TimelineBuilder:
    ...
    def page(self, when, who, path, title, kind, body, keys=False, **extra):
...
def sprint_events() -> list[Event]:
    ...
    The sprint board itself is not part of this story: it is regenerated once, at
    scaffold, by the owner running `docket check --fix` before any of the story is
    replayed \u2014 see the "Boards regenerated from the workflow" commit."""
    ingrid = PEOPLE["ingrid"]
    events = []
    for sp in SPRINTS:
    ...
    for sp in SPRINTS[:5]:
        ...
        events.append(page(end_when, ingrid, "docs/sprints/%s.md" % sp.name, sp.name,
                           SPRINT_PAGE, end_body, keys=True, **extra))
